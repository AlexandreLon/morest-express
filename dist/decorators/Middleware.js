...
function use(middlewareModel, path) {
    if (middlewareModel == null)
        return;
    global_1.app.use(path, (req, res, next) => middlewareModel.run(req, res, next));
}
function MiddlewareRoute(MiddlewareModelClass) {
...
function MiddlewareController(MiddlewareModelClass) {
    return function (Constructor) {
        if (global_1.bases[Constructor.name] != null)
            use(new MiddlewareModelClass(), global_1.bases[Constructor.name]);
        else
